...
public class OhjelmistoProjektiApplication {
	...
	public CommandLineRunner questionDemo(QuestionRepository qrepository, AnswerRepository arepository) {
		return (args) -> {
			...
			arepository.save(new Answer ("Kaksi vuotta", qrepository.findByQuestion("Monetta vuotta opiskelet Haaga-Heliassa?").get(0)));
			...
			arepository.save(new Answer ("Ei", qrepository.findByQuestion("Onko Haaga-Heliassa mielestäsi tarpeeksi visuaaliseen suunnitteluun ohjaavia kursseja?").get(0)));
			...
			arepository.save(new Answer ("Kyllä, Metropoliassa", qrepository.findByQuestion("Oletko suorittanut UX/UI suunnittelu kursseja jonkin muun koulun/palvelun kautta?").get(0)));
